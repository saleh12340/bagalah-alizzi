...
export async function saveThermalPrinter(device: ThermalPrinterDevice) {
  await AsyncStorage.multiSet([
    [PRINTER_ADDRESS_KEY, device.address],
    [PRINTER_NAME_KEY, device.name],
  ]);
}

export async function clearSavedThermalPrinter() {
  await AsyncStorage.multiRemove([PRINTER_ADDRESS_KEY, PRINTER_NAME_KEY]);
}

export async function isBluetoothEnabled() {
  if (Platform.OS !== "android") return false;
  return Boolean(await BluetoothManager.isBluetoothEnabled());
}
...
export async function disconnectThermalPrinter(address: string) {
  if (Platform.OS !== "android") return;
  try { await BluetoothManager.disconnect(address); } catch { /* already disconnected */ }
}
...
export async function printThermalReceipt(options: {
  ...
  await BluetoothEscposPrinter.printText("بقالة العزي للمواد الغذائية\n", { encoding: "UTF-8", widthtimes: 2, heigthtimes: 2, fonttype: 0 });
  await BluetoothEscposPrinter.printText("فاتورة مبيعات\n", { encoding: "UTF-8", widthtimes: 1, heigthtimes: 1, fonttype: 0 });
  await BluetoothEscposPrinter.printText(`${new Date().toLocaleDateString("ar-SA")}\n`, { encoding: "UTF-8" });
  await BluetoothEscposPrinter.printText(`${separator}\n`, { encoding: "UTF-8" });

  await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.LEFT);
  await BluetoothEscposPrinter.printText(`العميل: ${options.customerName || "نقدي"}\n`, { encoding: "UTF-8" });
  await BluetoothEscposPrinter.printText(`${separator}\n`, { encoding: "UTF-8" });

  for (const line of options.lines) {
    const lineTotal = Number(line.quantity || 0) * Number(line.unitPrice || 0);
    await BluetoothEscposPrinter.printText(`${line.name}\n`, { encoding: "UTF-8" });
    await BluetoothEscposPrinter.printText(`  ${line.quantity} × ${Number(line.unitPrice || 0).toFixed(2)} = ${lineTotal.toFixed(2)}\n`, { encoding: "UTF-8" });
  }

  await BluetoothEscposPrinter.printText(`${separator}\n`, { encoding: "UTF-8" });
  await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.RIGHT);
  await BluetoothEscposPrinter.printText(`الإجمالي: ${total.toFixed(2)} ر.س\n`, { encoding: "UTF-8", widthtimes: 2, heigthtimes: 2 });
  await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.CENTER);
...
export async function testThermalPrinter() {
  ...
  await BluetoothEscposPrinter.printerAlign(BluetoothEscposPrinter.ALIGN.CENTER);
  await BluetoothEscposPrinter.printText("بقالة العزي للمواد الغذائية\n", { encoding: "UTF-8", widthtimes: 2, heigthtimes: 2 });
  await BluetoothEscposPrinter.printText("اختبار الطابعة الحرارية\n", { encoding: "UTF-8" });
  await BluetoothEscposPrinter.printText(`${new Date().toLocaleString("ar-SA")}\n\n\n`, { encoding: "UTF-8" });
  return true;
}
