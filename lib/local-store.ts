...
export async function loadLocalState():Promise<LocalState>{try{const raw=await AsyncStorage.getItem(KEY);if(!raw){const seeded=seedState();await saveLocalState(seeded);return seeded;}const parsed=JSON.parse(raw) as LocalState;if(!Array.isArray(parsed.products)||!Array.isArray(parsed.customers))throw new Error("invalid");if(parsed.products.length===0&&parsed.customers.length===0){const seeded=seedState();await saveLocalState(seeded);return seeded;}return parsed;}catch{const seeded=seedState();await saveLocalState(seeded);return seeded;}}
