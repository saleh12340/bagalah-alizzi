...
function InvoiceView({state,saveState,products,customers,colors,storeName,onBack}:{state:LocalState;saveState:(s:LocalState)=>Promise<void>;products:LocalProduct[];customers:LocalCustomer[];colors:any;storeName:string;onBack:()=>void}) {
  ...
  const saveInvoice=async(showAlert=true)=>{
    const clean=lines.filter(x=>x.name.trim()&&x.quantity>0).map(x=>({...x,name:x.name.trim(),unitPrice:Number(x.unitPrice)||0}));
    const invoiceTotal=clean.reduce((s,x)=>s+x.quantity*x.unitPrice,0);
    if(!clean.length){Alert.alert("تنبيه","أدخل بندًا واحدًا على الأقل.");return null;}
    if(invoiceTotal<=0){Alert.alert("تنبيه","تأكد من إدخال القيمة الإجمالية.");return null;}
    const paidValue=Math.min(invoiceTotal,Math.max(0,Number(paid)||0));
    const next={...state,products:state.products.map(p=>({...p})),customers:state.customers.map(c=>({...c})),invoices:[...state.invoices]};
    ...
    const finalPhone=selectedCustomer?.phone||undefined;
    const id=editId||next.nextId++;
    const invoice:LocalInvoice={id,invoiceNo:old?.invoiceNo||("F-"+Date.now()),date:now(),customerId:cid,customerName:finalName,total:invoiceTotal,paid:paidValue,items:clean};
    ...
    if(cid){const customer=next.customers.find(c=>c.id===cid);if(customer)customer.balance=Math.max(0,customer.balance)+(invoiceTotal-paidValue);}
    try{await saveState(next);}catch(e:any){Alert.alert("خطأ في الحفظ",e?.message||"تعذر حفظ الفاتورة على الجهاز.");return null;}
    if(showAlert) Alert.alert("تم الحفظ","تم حفظ الفاتورة على الجهاز.");
    return {lines:clean,customerName:finalName||undefined,customerPhone:finalPhone,total:invoiceTotal};
  };
  const save=async()=>{const result=await saveInvoice(true);if(result)reset();};
  const remove=async(id:number)=>{const inv=state.invoices.find(i=>i.id===id);if(!inv)return;const next={...state,products:state.products.map(p=>({...p})),invoices:state.invoices.filter(i=>i.id!==id),customers:state.customers.map(c=>({...c}))};for(const item of inv.items){const p=next.products.find(p=>p.id===item.productId);if(p)p.stock+=item.quantity;}if(inv.customerId){const customer=next.customers.find(c=>c.id===inv.customerId);if(customer)customer.balance=Math.max(0,customer.balance-(inv.total-inv.paid));}await saveState(next);if(editId===id)reset();};
  const doPrint=async()=>{try{const result=await saveInvoice(false);if(!result)return;await printReceipt(result.lines,result.customerName,"80mm");reset();}catch(e:any){Alert.alert("خطأ في الطباعة",e?.message||"تعذر الطباعة.");}};
  const doShare=async()=>{try{const result=await saveInvoice(false);if(!result)return;const uri=await shareInvoicePdf(result.lines,result.customerName,result.customerPhone,"80mm",{storeName});if(uri){Alert.alert("تمت المشاركة","تم تجهيز الفاتورة للمشاركة كملف PDF.");reset();}}catch(e:any){Alert.alert("خطأ في المشاركة",e?.message||"تعذر مشاركة الفاتورة.");}};  return <View>
    ...
    <Card>
      ...
      {lines.map((l,idx)=><View key={idx} style={[styles.invoiceLine,{borderColor:colors.border,backgroundColor:colors.background}]}>
        ...
        <View style={styles.invoiceInputRow}>
          <TextInput value={String(l.quantity*l.unitPrice||"")} onChangeText={t=>{const totalValue=Number(t.replace(",","."))||0;const q=l.quantity||1;updateLine(idx,{unitPrice:q?totalValue/q:0})}} keyboardType="decimal-pad" placeholder="الإجمالي" style={[styles.cellInput,styles.totalInput,{color:colors.foreground,borderColor:colors.border}]}/>
          <TextInput value={String(l.quantity)} onChangeText={t=>updateLine(idx,{quantity:Math.max(0,Number(t.replace(",","."))||0)})} keyboardType="decimal-pad" style={[styles.cellInput,styles.quantityInput,{color:colors.foreground,borderColor:colors.border}]}/>
          <TextInput value={l.name} onChangeText={t=>{const p=products.find(p=>p.name.trim()===t.trim());updateLine(idx,{name:t,productId:p?.id||0,unitPrice:p?.salePrice||l.unitPrice})}} placeholder="اسم الصنف" style={[styles.cellInput,styles.nameInput,{flex:1.35,color:colors.foreground,borderColor:colors.border}]}/>
